function [mpc_obj, Ad, Bd, Cd] = compute_mpc(x0, R, p)
    ...
    %% Discretise
    Ts         = 0.01; %  Down-up =0.001 up-up=0.01
    sys_c      = ss(A, B, C, D);
    ...
    %% Horizons
    Np = 30;   % down-up 500 up-up=30 % prediction horizon — longer = better but slower
    Nc = 3;  % down-up 30 up-up=3 % control horizon
    ...
    mpc_obj.Weights.ManipulatedVariables     = 3; %down-up 10 up-up=3

    % ManipulatedVariablesRate: penalty on Δu (change in u)
    % Higher = smoother motor commands
    mpc_obj.Weights.ManipulatedVariablesRate = 0.01; %down-up 1 up-up=0.01
    ...
    mpc_obj.Weights.OutputVariables = [30, 150];    %down-up=100,1000 up-up=30,150
